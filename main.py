...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    output = tf.layers.conv2d_transpose(output, num_classes, 4, 2, padding='same', kernel_regularizer=REGULARIZER)

    pool4_out = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='same', kernel_regularizer=REGULARIZER)
    pool4_out = tf.multiply(pool4_out, 1e-2, name='pool4_out_scaled')
    output = tf.add(output, pool4_out)
    output = tf.layers.conv2d_transpose(output, num_classes, 4, 2, padding='same', kernel_regularizer=REGULARIZER)
    
    pool3_out = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding='same', kernel_regularizer=REGULARIZER)
